...
/*   Updated: 2025/07/02 12:41:53 by redei-ma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_ceiling_column(t_data *data, int x, int ceiling_end)
{
	...
	y = 0;
	printf("disegna il cielo\n");
	while (y < ceiling_end)
	{
		put_pixel_to_image(data->game->img, x, y, data->elements->ceiling_colours);
	}
	printf("fine cielo\n");
}
...
static void	draw_floor_column(t_data *data, int x, int floor_start)
{
	...
	while (y < WIN_HEIGHT)
	{
		put_pixel_to_image(data->game->img, x, y, data->elements->floor_colours);
		y++;
	}
}
